...
# ---- DELETE /document/{doc_id} (Step 2) -----------------------------------
 
 
def _ingest_one(client, auth_headers):
    response = client.post(
        "/ingest", files={"file": ("test.pdf", _minimal_pdf_bytes(), "application/pdf")}, headers=auth_headers
    )
    assert response.status_code == 200, response.text
...
def test_delete_document_returns_204_then_document_is_gone(client, mocked_chat, auth_headers):
    doc_id = _ingest_one(client, auth_headers)
 
    assert client.delete(f"/document/{doc_id}", headers=auth_headers).status_code == 204
    assert client.get(f"/document/{doc_id}", headers=auth_headers).status_code == 404
 
 
def test_delete_document_of_another_user_is_404_and_changes_nothing(client, mocked_chat, auth_headers):
    doc_id = _ingest_one(client, auth_headers)
    other = client.post(
        "/auth/register", json={"email": f"{uuid.uuid4()}@example.com", "password": "a-real-password-8plus"}
    ).json()["access_token"]
 
    response = client.delete(f"/document/{doc_id}", headers={"Authorization": f"Bearer {other}"})
 
    assert response.status_code == 404
    assert client.get(f"/document/{doc_id}", headers=auth_headers).status_code == 200
 
 
def test_delete_document_requires_login(client):
    assert client.delete("/document/anything").status_code == 401
